...
- [x] Otimização das Listas Curadas: Expansão do grid de emojis categorizados em Scroll Horizontal no New/Edit. Lógica aprimorada de sincronização no `EditListScreen` utilizando `originalPostIds` para calcular perfeitamente o Delta (add/remove) assegurando as chamadas corretas de `POST` e `DELETE` em paralelo. O `ListDetailScreen` força refresh na volta assegurando UI sempre responsiva a alterações no banco.
