...
- [x] Deleção de Listas: Foi movido e adicionado um modelo discreto de botão de "Excluir Lista" no final do form no `EditListScreen` resolvendo perfeitamente o encerramento da lista através da API com tratamentos de erro adequados (`try/catch`) seguido de redirecionamento para o Perfil com Deep Linking interno via Navigation Payload explícito para evitar bugs no stack.
